...
class StateValueNetwork:
    def __init__(self, state_size, learning_rate, name='state_value_network'):
        ...
        with tf.variable_scope(name):
            ...
            self.W2 = tf.get_variable("W2", [8, 8], initializer=tf2_initializer)
            self.b2 = tf.get_variable("b2", [8], initializer=tf2_initializer)
            self.W3 = tf.get_variable("W3", [8, 1], initializer=tf2_initializer)
...
def run(verbose=False):
    ...
    state_value = StateValueNetwork(state_size, learning_rate)
    ...
    with tf.Session() as sess:
        ...
        for episode in range(max_episodes):
            ...
            for t, transition in enumerate(episode_transitions):
                ...
                advantage = total_discounted_return - sess.run(state_value.output,
                                                               {state_value.state: transition.state})

                feed_dict_baseline = {state_value.state: transition.state,
                                      state_value.baseline: np.array(total_discounted_return).reshape(1)}
